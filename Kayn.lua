...
do
    ...
    local Files = {
        Lua = {
            Path = SCRIPT_PATH,
            Name = "Kayn.lua",
            Url = "https://raw.githubusercontent.com/miragessee/GoSKaynExt/master/Kayn.lua"
        },
        Version = {
            Path = SCRIPT_PATH,
            Name = "mirageskayn.version",
            Url = "https://raw.githubusercontent.com/miragessee/GoSKaynExt/master/mirageskayn.version"
        }
    }
...
function Kayn:QDmg(target)
    if self.KaynMenu.Choose_Kayn_Darkin.DarkinKayn:Value() then
        return QDmgR(target)
    else
        if myHero:GetSpellData(_Q).level == 0 then
            local Dmg1 = (({60, 80, 100, 120, 140})[1] + 0.65 * myHero.bonusDamage)
            return Dmg1
        else
            local Dmg1 = (({60, 80, 100, 120, 140})[myHero:GetSpellData(_Q).level] + 0.65 * myHero.bonusDamage)
            return Dmg1
        end
    end
end
...
function QDmgR(target)
    local Bonus = 0.04 * (myHero.bonusDamage / 100)
    local Rhaast = (0.5 * myHero.totalDamage) + ((0.05 + Bonus) * target.maxHealth)
    return Rhaast + Bonus
end

function WDmg()
    if myHero:GetSpellData(_W).level == 0 then
        local Dmg1 = (({90, 135, 180, 225, 270})[1] + 1.30 * myHero.bonusDamage)
        return Dmg1
    else
        local Dmg1 = (({90, 135, 180, 225, 270})[myHero:GetSpellData(_W).level] + 1.30 * myHero.bonusDamage)
        return Dmg1
    end
end
...
function Kayn:RDmg(target)
    if self.KaynMenu.Choose_Kayn_Darkin.DarkinKayn:Value() then
        ...
        if myHero:GetSpellData(_R).level == 0 then
            local Dmg1 = (({150, 250, 350})[1] + 1.75 * myHero.bonusDamage)
            return Dmg1
        else
            local Dmg1 = (({150, 250, 350})[myHero:GetSpellData(_R).level] + 1.75 * myHero.bonusDamage)
            return Dmg1
        end
    end
end

function RDmgR(target)
    local Rhaast = (0.1 + (0.13 * (myHero.bonusDamage / 100))) * target.maxHealth
    return Rhaast
end

local HeroIcon = "https://www.mobafire.com/images/champion/icon/kayn.png"
local IgniteIcon = "http://pm1.narvii.com/5792/0ce6cda7883a814a1a1e93efa05184543982a1e4_hq.jpg"
local QIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/e/e0/Reaping_Slash.png"
local WIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/6/6d/Blade%27s_Reach.png"
local EIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/d/d4/Shadow_Step.png"
local RIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/8/8d/Umbral_Trespass.png"
...
function Kayn:LoadMenu()
    ...
    self.KaynMenu = MenuElement({type = MENU, id = "Kayn", name = "Mirage's Kayn", leftIcon = HeroIcon})
    
    self.KaynMenu:MenuElement({id = "Choose_Kayn_Darkin", name = "Choose_Kayn_Darkin", type = MENU})
    self.KaynMenu.Choose_Kayn_Darkin:MenuElement({id = "DKayn", name = "Default Kayn", value = false, leftIcon = DefaultKaynIcon})
    self.KaynMenu.Choose_Kayn_Darkin:MenuElement({id = "AKayn", name = "Assasin Kayn", value = false, leftIcon = AssasinKaynIcon})
    self.KaynMenu.Choose_Kayn_Darkin:MenuElement({id = "DarkinKayn", name = "DarkinKayn", value = true, leftIcon = DarkinIcon})
    
    self.KaynMenu:MenuElement({id = "Harass", name = "Harass", type = MENU})
    self.KaynMenu.Harass:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.KaynMenu.Harass:MenuElement({id = "UseW", name = "Use W", value = true, leftIcon = WIcon})
    
    self.KaynMenu:MenuElement({id = "Combo", name = "Combo", type = MENU})
    self.KaynMenu.Combo:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.KaynMenu.Combo:MenuElement({id = "UseW", name = "Use W", value = true, leftIcon = WIcon})
    self.KaynMenu.Combo:MenuElement({id = "UseR", name = "Use R", value = false, leftIcon = RIcon})
    self.KaynMenu.Combo:MenuElement({id = "UseRl", name = "Use R least health enemy", value = false, leftIcon = RIcon})
    self.KaynMenu.Combo:MenuElement({id = "UseRKS", name = "Use R is killable", value = true, leftIcon = RIcon})
    self.KaynMenu.Combo:MenuElement({id = "UseSmite", name = "Use smite", value = true, leftIcon = SmiteIcon})

    self.KaynMenu:MenuElement({id = "KillSteal", name = "KillSteal", type = MENU})
    self.KaynMenu.KillSteal:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.KaynMenu.KillSteal:MenuElement({id = "UseW", name = "Use W", value = true, leftIcon = EIcon})
    self.KaynMenu.KillSteal:MenuElement({id = "UseR", name = "Use R", value = true, leftIcon = RIcon})
    self.KaynMenu.KillSteal:MenuElement({id = "UseIgnite", name = "Use Ignite", value = true, leftIcon = IgniteIcon})
    
    self.KaynMenu:MenuElement({id = "Clear", name = "Clear", type = MENU})
    self.KaynMenu.Clear:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.KaynMenu.Clear:MenuElement({id = "UseW", name = "Use W", value = true, leftIcon = EIcon})
    
    self.KaynMenu:MenuElement({id = "AutoLevel", name = "AutoLevel", type = MENU})
    self.KaynMenu.AutoLevel:MenuElement({id = "AutoLevel", name = "First Q->E->W then Q->W->E", value = true})
    
    self.KaynMenu:MenuElement({id = "Escape", name = "Escape", type = MENU})
    self.KaynMenu.Escape:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = HPIcon})
    
    self.KaynMenu:MenuElement({id = "Drawings", name = "Drawings", type = MENU})
    self.KaynMenu.Drawings:MenuElement({id = "DrawQ", name = "Draw Q Range", value = true})
    self.KaynMenu.Drawings:MenuElement({id = "DrawW", name = "Draw W Range", value = true})
    self.KaynMenu.Drawings:MenuElement({id = "DrawR", name = "Draw R Range", value = true})
    self.KaynMenu.Drawings:MenuElement({id = "DrawAA", name = "Draw Killable AAs", value = false})
    self.KaynMenu.Drawings:MenuElement({id = "DrawJng", name = "Draw Jungler Info", value = true})
    self.KaynMenu.Drawings:MenuElement({id = "DrawKS", name = "Draw Killable Skills Info", value = true})
    self.KaynMenu.Drawings:MenuElement({id = "DrawKillable", name = "Draw Killable Enemy Info", value = true})
    
    self.KaynMenu:MenuElement({id = "Items", name = "Items", type = MENU})
    self.KaynMenu.Items:MenuElement({id = "UseBC", name = "Use Bilgewater Cutlass", value = true})
    self.KaynMenu.Items:MenuElement({id = "UseBOTRK", name = "Use BOTRK", value = true})
    self.KaynMenu.Items:MenuElement({id = "OI", name = "%HP To Use Offensive Items", value = 35, min = 0, max = 100, step = 5})
    self.KaynMenu.Items:MenuElement({id = "YG", name = "Use Youmuu's Ghostblade", value = true})
    
    self.KaynMenu:MenuElement({id = "blank", type = SPACE, name = ""})
    self.KaynMenu:MenuElement({id = "blank", type = SPACE, name = "Script Ver: " .. Version .. " - LoL Ver: " .. LVersion .. ""})
    self.KaynMenu:MenuElement({id = "blank", type = SPACE, name = "by " .. Author .. ""})
end

function Kayn:LoadSpells()
    KaynQ = {delay = 0.15, speed = math.huge, radius = 350, range = 350}
    KaynW = {range = 700, rangeK = 900, speed = math.huge, delay = 0.55, radius = 90}
    KaynR = {range = 550, rangeK = 750}
...
function Kayn:Tick()
    ...
    if self.KaynMenu.AutoLevel.AutoLevel:Value() then
...
function Kayn:Items()
    ...
    if targetBC then
        if (targetBC.health / targetBC.maxHealth) * 100 <= self.KaynMenu.Items.OI:Value() then
            if self.KaynMenu.Items.UseBC:Value() then
    ...
    if targetBOTRK then
        if (targetBOTRK.health / targetBOTRK.maxHealth) * 100 <= self.KaynMenu.Items.OI:Value() then
            if self.KaynMenu.Items.UseBOTRK:Value() then
    ...
    local target = GOS:GetTarget(850, "AD")
    
    if target then
        if self.KaynMenu.Items.YG:Value() then
            if GetItemSlot(myHero, 3142) > 0 and ValidTarget(target, 550) then
                if myHero:GetSpellData(GetItemSlot(myHero, 3142)).currentCd == 0 then
                    Control.CastSpell(Item_HK[GetItemSlot(myHero, 3142)], target)
                end
            end
        end
    end
...
function Kayn:CollisionX(myHeroPos, dangerousPos, unitPos, radius)
...
function Kayn:CalculateEndPos(startPos, placementPos, unitPos, range)
    if range > 0 then
...
function Kayn:ProcessSpell(units)
    for i = 1, #units do
        local unit = units[i]
    ...
    for key, v in pairs(IS) do
        ...
        if GetDistanceSqr(SpellHit, SpellPosition) <= GetDistanceSqr(dodge, SpellPosition) and GetDistance(SpellHit, v.sPos) - v.radius - myHero.boundingRadius <= GetDistance(v.sPos, v.ePos) then
            if GetDistanceSqr(myHero.pos, SpellHit) < (v.radius + myHero.boundingRadius) ^ 2 then
                if self.KaynMenu.Escape.UseQ:Value() then
                    if IsReady(_Q) then
                        local castPos = myHero.pos + Vector(myHero.pos, v.sPos):Normalized() * 100
                        Control.CastSpell(HK_Q, castPos * -1)
                    end
                end
            end
        end
...
function Kayn:KillSteal()
    if self.KaynMenu.KillSteal.UseIgnite:Value() then
        for i, enemy in pairs(GetEnemyHeroes()) do
    ...
    if self.KaynMenu.KillSteal.UseR:Value() then
        if IsReady(_R) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, self:GetRRange()) and enemy.health + enemy.shieldAP < self:RDmg(enemy) then
                    LocalControlCastSpell(HK_R, enemy)
                end
            end
        end
    end
    if self.KaynMenu.KillSteal.UseQ:Value() then
        if IsReady(_Q) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, KaynQ.range) and enemy.health + enemy.shieldAP < self:QDmg(enemy) then
                    LocalControlCastSpell(HK_Q, enemy)
                end
            end
        end
    end
    if self.KaynMenu.KillSteal.UseW:Value() then
        if IsReady(_W) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, self:GetWRange()) and enemy.health + enemy.shieldAP < WDmg() then
...
function Kayn:GetWRange()
    if self.KaynMenu.Choose_Kayn_Darkin.AKayn:Value() then
        return KaynW.rangeK
    else
        return KaynW.range
    end
...
function Kayn:Harass()
    --print(TestBuff(myHero))

    --print(myHero:GetSpellData(_Q).range)

    local targetW = GOS:GetTarget(self:GetWRange(), "AD")
    
    if targetW then
        if not IsImmune(targetW) then
            if self.KaynMenu.Harass.UseW:Value() then
                if IsReady(_W) then
                    if ValidTarget(targetW, self:GetWRange()) then
                        local hitChance, aimPosition = HPred:GetHitchance(myHero.pos, targetW, self:GetWRange(), KaynW.delay, KaynW.speed, KaynW.radius, false)
    ...
    local targetQ = GOS:GetTarget(KaynQ.range, "AD")
    
    if targetQ then
        if not IsImmune(targetQ) then
            if self.KaynMenu.Harass.UseW:Value() then
                if IsReady(_Q) and self.Collision == false then
                    if ValidTarget(targetQ, KaynQ.range) then
                        local hitChance, aimPosition = HPred:GetHitchance(myHero.pos, targetQ, KaynQ.range, KaynQ.delay, KaynQ.speed, KaynQ.radius, false)
                        if hitChance and hitChance >= 1 then
                            --if GetMinionCollision(myHero.pos, aimPosition, AkaliE.radius) == 0 then
...
function Kayn:CastW(target, EcastPos)
    if LocalGameTimer() - OnWaypoint(target).time > 0.05 and (LocalGameTimer() - OnWaypoint(target).time < 0.125 or LocalGameTimer() - OnWaypoint(target).time > 1.25) then
        if GetDistance(myHero.pos, EcastPos) <= self:GetWRange() then
            LocalControlCastSpell(HK_W, EcastPos)
        end
    end
end

function Kayn:CastQ(target, EcastPos)
    if LocalGameTimer() - OnWaypoint(target).time > 0.05 and (LocalGameTimer() - OnWaypoint(target).time < 0.125 or LocalGameTimer() - OnWaypoint(target).time > 1.25) then
        if GetDistance(myHero.pos, EcastPos) <= KaynQ.range then
...
function Kayn:Combo()
    ...
    if targetW then
        if not IsImmune(targetW) then
            if self.KaynMenu.Combo.UseW:Value() then
                if IsReady(_W) then
                    if ValidTarget(targetW, self:GetWRange()) then
                        local hitChance, aimPosition = HPred:GetHitchance(myHero.pos, targetW, self:GetWRange(), KaynW.delay, KaynW.speed, KaynW.radius, false)
                        if hitChance and hitChance >= 1 then
    ...
    if self.KaynMenu.Combo.UseSmite:Value() then
    ...
    local targetR = GOS:GetTarget(self:GetRRange(), "AD")

    if self.KaynMenu.Combo.UseR:Value() then
        if IsReady(_R) then
    ...
    if self.KaynMenu.Combo.UseRKS:Value() then
        if IsReady(_R) then
            if ValidTarget(targetR, self:GetRRange()) and targetR.health + targetR.shieldAP < self:RDmg(targetR) then
                LocalControlCastSpell(HK_R, targetR)
            end
        end
    end
    
    if self.KaynMenu.Combo.UseRl:Value() then
        local minHealth = 50000
        local minHealthEnemy
        
        for i, enemy in pairs(GetEnemyHeroes()) do
            if ValidTarget(enemy, self:GetRRange()) and enemy.health < minHealth then
...
function Kayn:Clear()
    if self.KaynMenu.Clear.UseW:Value() then
        for i = 1, LocalGameMinionCount() do
            local minion = LocalGameMinion(i)
            if minion and minion.isEnemy then
                if IsReady(_W) then
                    if ValidTarget(minion, self:GetWRange()) then
    ...
    if self.KaynMenu.Clear.UseQ:Value() then
        for i = 1, LocalGameMinionCount() do
            local minion = LocalGameMinion(i)
            if minion and minion.isEnemy then
                if IsReady(_Q) then
                    if ValidTarget(minion, KaynQ.range) then
...
function Kayn:Draw()
    ...
    if self.KaynMenu.Drawings.DrawQ:Value() then Draw.Circle(myHero.pos, KaynQ.range, 1, Draw.Color(255, 0, 191, 255)) end
    if self.KaynMenu.Drawings.DrawW:Value() then Draw.Circle(myHero.pos, self:GetWRange(), 1, Draw.Color(255, 65, 105, 225)) end
    if self.KaynMenu.Drawings.DrawR:Value() then Draw.Circle(myHero.pos, self:GetRRange(), 1, Draw.Color(255, 0, 0, 255)) end
    
    for i, enemy in pairs(GetEnemyHeroes()) do
        if self.KaynMenu.Drawings.DrawJng:Value() then
        ...
        if self.KaynMenu.Drawings.DrawAA:Value() then
            if ValidTarget(enemy) then
        ...
        if self.KaynMenu.Drawings.DrawKS:Value() then
            if ValidTarget(enemy) then
                if enemy.health < (self:QDmg(enemy)) then
                    Draw.Text("Killable Skills (Q): ", 25, enemy.pos2D.x - 38, enemy.pos2D.y + 10, Draw.Color(0xFFFF0000))
                elseif enemy.health < (self:QDmg(enemy) + WDmg()) then
                    Draw.Text("Killable Skills (Q+W): ", 25, enemy.pos2D.x - 38, enemy.pos2D.y + 10, Draw.Color(0xFFFF0000))
                elseif enemy.health < (self:QDmg(enemy) + WDmg() + self:RDmg(enemy)) then
                    Draw.Text("Killable Skills (Q+W+R): ", 25, enemy.pos2D.x - 38, enemy.pos2D.y + 10, Draw.Color(0xFFFF0000))
                end
            end
        end
        if self.KaynMenu.Drawings.DrawKillable:Value() then
            if ValidTarget(enemy) then
                if enemy.health < (self:QDmg(enemy)) then
                    Draw.Text(tostring(enemy.charName) .. " Killable Skills (Q): ", 25, myHero.pos2D.x - 38, myHero.pos2D.y + 10, Draw.Color(0xFFFF0000))
                elseif enemy.health < (self:QDmg(enemy) + WDmg()) then
                    Draw.Text(tostring(enemy.charName) .. " Killable Skills (Q+W): ", 25, myHero.pos2D.x - 38, myHero.pos2D.y + 10, Draw.Color(0xFFFF0000))
                elseif enemy.health < (self:QDmg(enemy) + WDmg() + self:RDmg(enemy)) then
                    Draw.Text(tostring(enemy.charName) .. " Killable Skills (Q+W+R): ", 25, myHero.pos2D.x - 38, myHero.pos2D.y + 10, Draw.Color(0xFFFF0000))
                end
            end
        end
    end
